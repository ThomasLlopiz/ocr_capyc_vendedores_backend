...
class ProcessOrderController extends Controller
{
    public function createOrder(Request $request)
    {
        ...
        $c5_moeda = $request->input('c5_moeda');
        $c5_moeda = (empty($c5_moeda) && $c5_moeda !== 0) ? 1 : (float) $c5_moeda;
        ...
        try {
            ...
            foreach ($articulos as $art) {
                $productCode = trim((string) ($art['product_code'] ?? ''));
                $quantity = (int) ($art['quantity'] ?? 0);
                ...
                if ($prcFront === null) {
                    ...
                    $unitPrice = (float) ($priceRow->da1_prcven ?? 0);
                } else {
                    $unitPrice = (float) $prcFront;
                }

                $conv = (float) ($art['c6_xconv'] ?? $product->b1_conv ?? 1);
                if ($conv <= 0) {
                    $conv = 1;
                }

                $unsven = round($quantity / $conv, 4);
